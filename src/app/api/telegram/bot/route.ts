  ...
  try {
    ...
    // Handle voice messages
      ...
      try {
        ...
        const voiceFile = new File([voiceBuffer], 'voice.ogg', { type: 'audio/ogg' });
        ...
        if (!transcription.text || transcription.text.trim() === '') {
          await sendTelegramMessage(chatId, 'I couldn\'t understand the voice message. Please try again.');
          return NextResponse.json({ success: true });
        }
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            { 
              role: 'system', 
              content: 'You are responding to a voice message. The user said: "' + transcription.text + '"' 
            },
